...
@section('content')
    ...
    <div>
        ...
        <ul class="nav nav-tabs" role="tablist">
            <li role="presentation" class="active"><a href="#hierarchy" aria-controls="home" role="tab" data-toggle="tab">Hierarchy</a></li>
        ...
        <div class="tab-content">
            <div role="tabpanel" class="tab-pane active" id="hierarchy" style="padding-top:20px">
                    ...
                    <ol class="dd-list">
                        @foreach ($groups as $group)
                            <li class="dd-item" data-id="{!! $group->id !!}">
                                <div class="dd-handle">{!! $group->name !!} <em>- (Parent Group)</em></div>

                            @if ($group->children->count())
                                <ol class="dd-list">
                                    @foreach ($group->children as $child)
                                        <li class="dd-item" data-id="{!! $child->id !!}">
                                            <div class="dd-handle">{!! $child->name !!} <em>- (Child Group)</em></div>
                                        </li>
                                    @endforeach
                                </ol>
                            </li>
            ...
            <div role="tabpanel" class="tab-pane" id="groups" style="padding-top:20px">
                    ...
                    <tbody>
                        @foreach ($groups as $group)
                            ...
                            @if ($group->children->count())
                                @foreach ($group->children as $child)
                                    <tr>
                                        <td style="padding-left:40px"><em>{!! $child->name !!}</em></td>
                ...
                <div class="pull-left">
                    {{ $groups->total() }} {{ trans('crud.permissions.groups.total') }}
                </div>
