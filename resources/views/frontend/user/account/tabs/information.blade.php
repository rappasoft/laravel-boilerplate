<x-forms.patch :action="route('frontend.user.profile.update')">
    <div class="form-group row">
        ...
        <div class="col-md-9">
            <input type="text" name="name" class="form-control mb-3" placeholder="{{ __('Name') }}" value="{{ old('name') ?? $logged_in_user->name }}" required autofocus autocomplete="name" />
        </div>
    </div><!--form-group-->
    ...
    @if ($logged_in_user->canChangeEmail())
        <div class="form-group row">
            ...
            <div class="col-md-9">
                ...
                <input type="email" name="email" id="email" class="form-control mb-3" placeholder="{{ __('E-mail Address') }}" value="{{ old('email') ?? $logged_in_user->email }}" required autocomplete="email" />
            </div>
        </div><!--form-group-->
    @endif
